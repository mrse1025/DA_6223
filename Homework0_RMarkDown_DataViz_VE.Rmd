...
```{r}
cpi_merge1 <- merge(month_map, cpi, by=c("month_name"))
print(cpi_merge1)
cpi_merge2 <- cpi_merge1[order(cpi_merge$year, cpi_merge1$month),]
cpi_merge3 <- cpi_merge2[1:6, ]
...
```{r}
housing_1 <- merge(txhousing, cpi_merge2, by=c("year", "month"))
housing_h <- housing_1[,c("city", "year", "month", "sales", "volume", "median", "listings", "inventory", "date", "cpi")]
housing <- housing_h[order(housing_h$city, housing_h$year, housing$month), ]
head(housing)
```
...
```{r}
housing$cpi_latest <- cpi_merge2[cpi_merge2$month==7 & cpi_merge2$year==2020,c("cpi")]

housing$multiplier <- housing$cpi_latest/housing$cpi

housing$volume_adj <- housing$volume * housing$multiplier

housing$median_adj <- housing$median * housing$multiplier
head(housing)
